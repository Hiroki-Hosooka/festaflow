...
export default async function AdminClassificationsPage({
  ...
  const { affiliation, area, genre } = await listAllClassificationOptions(auth.eventId);
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-[12.5px] text-[var(--muted)] mt-1 leading-relaxed">
          企画一覧の絞り込みや企画フォームで使う「所属区分」「開催エリア」「企画ジャンル」の選択肢を、学校の実情に合わせて自由に追加・変更・削除できます。削除・変更した選択肢が既に使われている企画では、その時点の表示名がそのまま残ります。企画ジャンルは提出項目（「提出項目」ページ）を特定のジャンルにだけ表示する出し分けにも使えます。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
        ...
        <OptionColumn eventSlug={eventSlug} category="genre" title="企画ジャンル" options={genre} />
      </div>
    </div>
  );
}
